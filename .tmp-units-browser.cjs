...
const profile = fs.mkdtempSync(path.join(os.tmpdir(), "codex-units-"));
...
const requests = [];
const browserErrors = [];
let units = [{ id: 1, name: "Caja", symbol: "cj", isActive: true }, { id: 2, name: "Pieza", symbol: "pza", isActive: true }, { id: 3, name: "Par", symbol: null, isActive: false }];
...
const getCount = () => requests.filter(r => r.method === "GET" && r.pathname.endsWith("/units")).length;
...
async function intercept(params) {
    ...
    if (request.method === "OPTIONS") body = {};
    ...
    else if (/\/units(?:\/\d+(?:\/status)?)?$/.test(pathname)) {
        requests.push({ method: request.method, pathname, payload: request.postData ? JSON.parse(request.postData) : null });
        ...
        if (request.method === "GET") {
            ...
            if (request.method === "POST") {
                body = { id: 4, ...payload, isActive: true }; units.push(body);
            } else {
                const id = Number(pathname.match(/\/units\/(\d+)/)[1]);
                body = { ...units.find(color => color.id === id), ...payload };
                units = units.map(color => color.id === id ? body : color);
            }
        }
    } else {
...
(async () => {
    ...
    await send("Page.navigate", { url: "http://127.0.0.1:" + server.address().port + "/units" });
    ...
    await screenshot("units-desktop");
    await change("unit-search", "cj");
    await until("document.querySelectorAll('tbody tr').length === 1");
    await change("unit-search", "");
    await change("unit-status", "inactive");
    await until("document.querySelectorAll('tbody tr').length === 1 && document.querySelector('tbody').textContent.includes('Par')");
    await click("Limpiar");
    await until("document.querySelectorAll('tbody tr').length === 3");
    assert.equal(getCount(), 1);
    await click("Nueva unidad de medida");
    await until("document.querySelector('dialog:modal') && document.activeElement.id === 'unit-name'");
    ...
    await change("unit-symbol", "temp");
    await change("unit-name", "  CAJA  ");
    ...
    assert.equal(await evaluate("document.activeElement.textContent.trim()"), "Nueva unidad de medida");
    await click("Nueva unidad de medida");
    await until("!!document.querySelector('dialog:modal')");
    assert.equal(await evaluate("document.getElementById('unit-name').value"), "");
    assert.equal(await evaluate("document.getElementById('unit-symbol').value"), "");
    await key("Escape", 27);
    await until("!document.querySelector('dialog')");
    assert.notEqual(await evaluate("document.body.style.overflow"), "hidden");
    await click("Nueva unidad de medida");
    await until("!!document.querySelector('dialog:modal')");
    await change("unit-name", "Metro");
    failMutation = true;
    await click("Crear unidad");
    ...
    await evaluate("window.__modalEvents=[];document.querySelector('dialog').addEventListener('cancel',event=>{window.__modalEvents.push({type:'cancel',time:Date.now(),busy:document.querySelector('dialog')?.getAttribute('aria-busy')});queueMicrotask(()=>window.__modalEvents.push({type:'afterCancel',prevented:event.defaultPrevented}));});");
    console.log('BEFORE_ESCAPE', await evaluate("({busy:document.querySelector('dialog')?.getAttribute('aria-busy'),time:Date.now()})"));
    await key("Escape", 27);
    console.log('AFTER_ESCAPE', await evaluate("({busy:document.querySelector('dialog')?.getAttribute('aria-busy'),time:Date.now(),events:window.__modalEvents,text:document.body.innerText.slice(-500)})"));
    ...
    await click("Editar");
    await until("!!document.querySelector('dialog:modal')");
    assert.equal(await evaluate("document.getElementById('unit-name').value"), "Caja");
    assert.equal(await evaluate("document.getElementById('unit-symbol').value"), "cj");
    await change("unit-symbol", " pkg ");
    await change("unit-name", "Caja grande");
    await click("Guardar cambios");
    await until("!document.querySelector('dialog') && document.querySelector('tbody').textContent.includes('Caja grande')");
    assert.deepEqual(requests.find(r => r.method === "POST").payload, { name: "Metro", symbol: null });
    assert.deepEqual(requests.find(r => r.method === "PUT").payload, { name: "Caja grande", symbol: "pkg" });
    await click("Desactivar");
    await until("document.querySelector('tbody tr:last-child').textContent.includes('Par') && [...document.querySelectorAll('tbody tr')].find(r=>r.textContent.includes('Caja grande')).textContent.includes('Activar')");
    assert.equal(getCount(), 1);
    ...
    await screenshot("units-mobile-modal");
    await click("Cancelar");
    units = [];
    await click("Actualizar");
    await until("document.body.textContent.includes('No hay colores registrados.')");
    viewer = true;
    units = [{ id: 1, name: "Caja", isActive: true }];
    await send("Page.reload");
    await until("document.querySelectorAll('tbody tr').length === 1");
    assert.equal(await evaluate("[...document.querySelectorAll('button')].some(b=>['Nueva unidad de medida','Editar','Desactivar'].includes(b.textContent.trim()))"), false);
